...
async def setup_form() -> str:
    ...
    lock_attr = " disabled aria-disabled='true'" if configured else ""
    lock_note = "<div class='card danger'><strong>Credentials are locked.</strong><p>Credentials can only be changed after <strong>Factory Reset</strong>. Factory Reset clears the local credential vault; it does not change your Chaster or EmlaLock accounts or locks.</p></div>" if configured else ""
    save_button = "<button type='submit' disabled>Credentials already saved</button>" if configured else "<button type='submit'>Save and start syncing</button>"
    body = """<div class='card'><h2>Setup</h2><p>Credentials are encrypted locally. Once the required credentials are saved, all credential fields are locked and cannot be changed until Factory Reset.</p></div>
""" + lock_note + """
<form method='post' action='/setup'>
<div class='card'><h3>Chaster</h3><label>Developer token (wearer / primary account)<br><input name='chaster_token' type='password' autocomplete='off' placeholder='Saved — locked until Factory Reset' required""" + lock_attr + """> </label><br><br><label>Lock ID<br><input name='chaster_lock_id' autocomplete='off' placeholder='Saved — locked until Factory Reset' required""" + lock_attr + """> </label><hr><h4>Chaster permissions required</h4><p><strong>For normal sync/add-time:</strong> the API token needs the <code>locks</code> scope and the lock contract must grant the token's account <strong>Add time</strong>.</p><p><strong>For subtract-time:</strong> the account making the Chaster request must have <strong>Remove time</strong>. Chaster's Standard preset normally grants Remove time to the keyholder, not the wearer.</p><label>Chaster keyholder access token (required if this bot must remove Chaster time as keyholder)<br><input name='chaster_keyholder_token' type='password' autocomplete='off' placeholder='Saved — locked until Factory Reset'""" + lock_attr + """></label><p><small>The keyholder token should belong to the Chaster keyholder account and be authorized with the <code>keyholder</code> scope. Leave it empty only if the primary token's account itself has Remove time.</small></p></div>
<div class='card'><h3>EmlaLock</h3><label>User ID<br><input name='emlalock_user_id' autocomplete='off' placeholder='Saved — locked until Factory Reset' required""" + lock_attr + """></label><br><br><label>API key<br><input name='emlalock_api_key' type='password' autocomplete='off' placeholder='Saved — locked until Factory Reset' required""" + lock_attr + """></label><br><br><label>Keyholder API key (required for subtract)<br><input name='emlalock_keyholder_api_key' type='password' autocomplete='off' placeholder='Saved — locked until Factory Reset' required""" + lock_attr + """></label></div>
<div class='card'><h3>Discord (optional)</h3><p>c-ebot uses Discord slash commands, so <strong>Message Content Intent is not required</strong>. The bot only requests the Guilds intent.</p><label>Bot token<br><input name='discord_bot_token' type='password' autocomplete='off' placeholder='Saved — locked until Factory Reset'""" + lock_attr + """></label><br><br><label>Application / Client ID<br><input name='discord_application_id' inputmode='numeric' autocomplete='off' placeholder='Saved — locked until Factory Reset'""" + lock_attr + """></label><br><br><label>Server / Guild ID<br><input name='discord_guild_id' inputmode='numeric' autocomplete='off' placeholder='Saved — locked until Factory Reset'""" + lock_attr + """></label><br><br><label>Log / alert channel ID<br><input name='discord_channel_id' inputmode='numeric' autocomplete='off' placeholder='Saved — locked until Factory Reset'""" + lock_attr + """></label><br><br><label>Admin user IDs<br><input name='discord_admin_user_ids' autocomplete='off' placeholder='Saved — locked until Factory Reset'""" + lock_attr + """></label><p><small>Admin IDs control /sync, /addtime, /subtracttime and /logs.</small></p>
...
async def setup_submit(chaster_token: str = Form(""), chaster_lock_id: str = Form(""), chaster_keyholder_token: str = Form(""), emlalock_user_id: str = Form(""), emlalock_api_key: str = Form(""), emlalock_keyholder_api_key: str = Form(""), discord_bot_token: str = Form(""), discord_application_id: str = Form(""), discord_guild_id: str = Form(""), discord_channel_id: str = Form(""), discord_admin_user_ids: str = Form("")):
    if has_secrets():
        return HTMLResponse(page("Setup locked", "<div class='card danger'><h2>Setup is locked</h2><p>Credentials have already been saved. They cannot be changed until you complete Factory Reset.</p><a href='/setup'>Back to Setup</a></div>"), status_code=409)
